...
- 基于稳定文章链接去重，只推送**先前未抓取过**的新闻；仅关注最近 `lookback_hours`（默认 24h）内发布的新闻，**不会反复向前翻旧闻**；**去重记录按目标群独立保存**，各群各自维护推送历史，互不影响
...
| `lookback_hours` | int | 24 | 只推送最近 N 小时内发布的新闻，避免反复向前翻旧闻 |
...
| `/csnews status` | 查看插件运行状态，及**当前群**的已推送记录数 |
| `/csnews reset` | 清空**当前群**的已推送记录，下次轮询重新推送窗口内新闻 |
